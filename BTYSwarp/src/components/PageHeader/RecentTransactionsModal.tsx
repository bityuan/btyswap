...
import { getBityuanScanLink } from 'utils'
...
const RecentTransactionsModal = ({ onDismiss = defaultOnDismiss, translateString }: RecentTransactionsModalProps) => {
  ...
  return (
    <Modal title={translateString('recentTransactions')} onDismiss={onDismiss}>
      {!account && (
          ...
          return (
            <>
              <Flex key={hash} alignItems="center" justifyContent="space-between" mb="4px">
                <LinkExternal href={getBityuanScanLink(chainId, hash, 'transaction')} color={color}>
                  {summary ?? hash}
                </LinkExternal>
                {icon}
              </Flex>
