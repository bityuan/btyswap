...
const SlippageToleranceSettings = ({ translateString }: SlippageToleranceSettingsModalProps) => {
  ...
  useEffect(() => {
    try {
      ...
      if (!Number.isNaN(rawValue) && rawValue > 0 && rawValue < MAX_SLIPPAGE) {
        ...
        setError(translateString('enterValidSlippagePercentage'))
      }
    } catch {
      setError(translateString('enterValidSlippagePercentage'))
    }
  ...
  useEffect(() => {
    if (userSlippageTolerance < RISKY_SLIPPAGE_LOW) {
      setError(translateString('transactionMayFail'))
    } else if (userSlippageTolerance > RISKY_SLIPPAGE_HIGH) {
      setError(translateString('transactionMayBeFrontrun'))
    }
  ...
  return (
    <Box mb="16px">
      <Flex alignItems="center" mb="8px">
        <Text bold>{translateString('slippageTolerance')}</Text>
        <QuestionHelper
          text={translateString('transactionRevertPriceChange')}
        />
      </Flex>
      <Options>
