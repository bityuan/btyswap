...
import { JSBI, Percent, Router, SwapParameters, Trade, TradeType } from '@btyswap-libs/sdk'
          ...
          swapCalls.map((call) => {
            const {
              ...
              contract,
                         ...
                         const options = !value || isZero(value) ? {} : { value: value.toString() }
              ...
              .catch((gasError) => {
                console.info('Gas estimate failed, trying eth_call to extract error', call)
                 ...
                 if (value && !isZero(value)) {
                   ...
                   callOptions = { value: valueStr }
                 }
                 return contract.callStatic[methodName](...args, callOptions)
                  .then((result) => {
