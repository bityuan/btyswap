...
export default function AddLiquidity({
  ...
  async function onAdd() {
    ...
    try {
      ...
      addTransaction(response, {
        summary: `Add ${parsedAmounts[Field.CURRENCY_A]?.toSignificant(3)} ${getTokenSymbol(currencies[Field.CURRENCY_A], chainId)} and ${parsedAmounts[Field.CURRENCY_B]?.toSignificant(3)} ${getTokenSymbol(currencies[Field.CURRENCY_B], chainId)}`,
      })
      ...
      <AutoColumn gap="20px">
        ...
        <Row>
          <UIKitText fontSize="24px">
            {`${currencies[Field.CURRENCY_A]?.symbol || (chainId === 2999 as any ? 'BTY' : 'BTY')}/${currencies[Field.CURRENCY_B]?.symbol || (chainId === 2999 as any ? 'BTY' : 'BTY')} Pool Tokens`}
          </UIKitText>
        </Row>
  ...
  const pendingText = `Supplying ${parsedAmounts[Field.CURRENCY_A]?.toSignificant(6)} ${getTokenSymbol(currencies[Field.CURRENCY_A], chainId)} and ${parsedAmounts[Field.CURRENCY_B]?.toSignificant(6)} ${getTokenSymbol(currencies[Field.CURRENCY_B], chainId)}`
  ...
  const handleCurrencyBSelect = useCallback(
    (currB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        ...
        history.push(`/add/${currencyIdA || (chainId === 2999 as any ? 'BTY' : 'BNB')}/${newCurrencyIdB}`)
      }
    },
            ...
            <AutoColumn gap="20px">
              {noLiquidity && (
                <ColumnCenter>
                  <Pane>
                    <AutoColumn gap="12px">
                      <UIKitText>{t('youAreFirstLiquidityProvider')}</UIKitText>
                      <UIKitText>
                        {t('ratioTokensSetPrice')}
                      </UIKitText>
                      <UIKitText>
                        {t('onceHappyRateClickSupply')}
                      </UIKitText>
                    </AutoColumn>
                  </Pane>
                </ColumnCenter>
