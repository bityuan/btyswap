...
import { useNormalLocksForUser, useAllNormalLocks, LockInfo } from '../../hooks/useLocks'
...
export default function TokenLocks() {
  const { t } = useTranslation()
  const { account } = useActiveWeb3React()
  const [activeSubTab, setActiveSubTab] = useState<TokenLockSubTab>(TokenLockSubTab.MY_LOCKS)
  const [selectedLock, setSelectedLock] = useState<LockInfo | null>(null)
  const [showDetailModal, setShowDetailModal] = useState(false)

  const { locks: myLocks, loading: myLocksLoading, error: myLocksError } = useNormalLocksForUser(account)
  const { locks: allLocks, loading: allLocksLoading, error: allLocksError } = useAllNormalLocks()

  const locks = activeSubTab === TokenLockSubTab.ALL ? allLocks : myLocks
  const loading = activeSubTab === TokenLockSubTab.ALL ? allLocksLoading : myLocksLoading
  const error = activeSubTab === TokenLockSubTab.ALL ? allLocksError : myLocksError
  ...
  const handleTabChange = (index: number) => {
    const tab = index === 0 ? TokenLockSubTab.MY_LOCKS : TokenLockSubTab.ALL
    setActiveSubTab(tab)
  }
    ...
    <>
      <CardBody>
        <AutoColumn gap="20px">
          <StyledButtonMenu>
            <TabButton
              active={activeSubTab === TokenLockSubTab.MY_LOCKS}
              onClick={() => handleTabChange(0)}
            >
              {t('myLocks')}
            </TabButton>
            <TabButton
              active={activeSubTab === TokenLockSubTab.ALL}
          ...
          {activeSubTab === TokenLockSubTab.MY_LOCKS && !account ? (
            ...
            <TableContainer>
              <TableHeader>
                ...
                <TableHeaderCell flex={1}>
                  <Text fontWeight="600">{t('tokenAddress')}</Text>
                </TableHeaderCell>
                <TableHeaderCell flex={1}>
